...
export default function PlayerTurn({ state, dispatch }: Props) {
  ...
  function onSelect(index: number) {
    if (round.turnStep === "picking") dispatch({ type: "TAKE_BRIEFCASE", index });
    else if (round.turnStep === "swapping") dispatch({ type: "BLIND_SWAP", index });
  }
  ...
  return (
    <div className="min-h-full pb-16">
      ...
      {round.turnStep === "swapping" ? (
        <div className="sticky top-[92px] z-10 flex items-center justify-center gap-3 bg-brass/15 px-4 py-2 text-sm text-paper">
      ...
      <div className="mx-auto max-w-3xl px-4 py-6">
        <BriefcaseGrid round={round} players={state.players} mode={gridMode} onSelect={onSelect} />
      </div>
